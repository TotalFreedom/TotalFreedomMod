...
public class Command_execute extends FreedomCommand
{
    ...
    @Override // uses the TFM announce command base.
    protected boolean run(CommandSender sender, Player playerSender, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        if (args.length < 1)
        {
            ...
            // instead of doing a Announce all arguments put on Execute will be logged as Warning in logs and it doesn't run any execute arguments.
