...
public class Command_verify extends FreedomCommand {
    ...
    protected boolean run(CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        ...
        if (args.length == 0) {
            playerMsg("TotalFreedomMod verification system", ChatColor.GOLD);
            StringBuilder line = new StringBuilder();
            while (line.length() < ChatPaginator.GUARANTEED_NO_WRAP_CHAT_PAGE_WIDTH) {
                line.append("-");
            }
            playerMsg(line.toString());

            playerMsg("By setting a password and using a OTP code generator,", ChatColor.GOLD);
            playerMsg("you can verify yourself if you become an impostor.", ChatColor.GOLD);
            playerMsg("Note: For this to work, you need to have a OTP generator", ChatColor.GOLD);
            playerMsg("such as Google Authenticator installed.", ChatColor.GOLD);
            playerMsg("");
            playerMsg("Password set: " + (verify.hasPassword() ? ChatColor.GREEN + "yes" : ChatColor.RED + "no"));
            playerMsg("OTP set: " + (verify.hasTotpSecret() ? ChatColor.GREEN + "yes" : ChatColor.RED + "no"));
            playerMsg("");
            playerMsg("You can set your password and OTP code generator with /verify set <password | otp>");
        }
